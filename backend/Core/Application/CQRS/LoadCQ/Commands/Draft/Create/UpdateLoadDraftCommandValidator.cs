...
/// <summary>
/// Валидатор для обновления черновика груза.
/// Наследует логику черновика: все поля опциональны, кроме Id и UserId.
/// </summary>
public class UpdateLoadDraftCommandValidator : AbstractValidator<UpdateLoadDraftCommand>
{
    public UpdateLoadDraftCommandValidator()
    {
        RuleFor(x => x.UserId)
            .Must(x => x != Guid.Empty)
        ...
        RuleForEach(x => x.RoutePoints)
            .SetValidator(new RoutePointDraftUpdateDtoValidator())
            .When(x => x.RoutePoints != null);

        RuleForEach(x => x.Payloads)
            .SetValidator(new PayloadDraftUpdateDtoValidator())
            .When(x => x.Payloads != null);
    }
}


public class RoutePointDraftUpdateDtoValidator : AbstractValidator<RoutePointDraftUpdateDto>
{
    public RoutePointDraftUpdateDtoValidator()
    {
        RuleFor(x => x.City)
...
public class PayloadDraftUpdateDtoValidator : AbstractValidator<PayloadDraftUpdateDto>
{
    public PayloadDraftUpdateDtoValidator()
    {
        RuleFor(x => x.Type)
            .NotEmpty()

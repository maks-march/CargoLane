...
export const DetailRightPanel: React.FC<Props> = ({ load }) => {
  ...
  // ИСПРАВЛЕНО: Безопасно достаем Display Name, Company Name и Аватарку
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const s = shipper as any;
  const shipperDisplayName = s?.displayName || s?.firstName || s?.companyName || 'Verified Shipper';
  const shipperCompany = s?.companyName || s?.country || 'Location not specified';
  const shipperAvatarUrl = s?.avatarPath || s?.avatarUrl;
  const shipperInitials = shipperDisplayName.substring(0, 2).toUpperCase();
  ...
  return (
    <aside className="detail-right-panel" style={{ width: '320px', display: 'flex', flexDirection: 'column', gap: '20px', flexShrink: 0 }}>
      ...
      <div className="detail-card" style={{ padding: '24px', background: 'white', borderRadius: '12px', border: '1px solid #E6E8EE' }}>
        ...
        {/* ИСПРАВЛЕНО: Выводим Display Name и Company Name, а также Аватарку */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
          <div style={{ width: '48px', height: '48px', background: '#EEF1FF', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3D5AFE', fontWeight: 600, fontSize: '18px', overflow: 'hidden', flexShrink: 0 }}>
            {shipperAvatarUrl ? (
               <img src={shipperAvatarUrl} alt="Avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ) : (
               shipperInitials
            )}
          </div>
          <div style={{ overflow: 'hidden' }}>
            <div style={{ fontWeight: 600, color: '#0E1116', fontSize: '15px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{shipperDisplayName}</div>
            <div style={{ fontSize: '13px', color: '#5C6470', marginTop: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{shipperCompany}</div>
          </div>
        </div>

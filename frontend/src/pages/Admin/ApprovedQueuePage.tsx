...
export const ApprovedQueuePage: React.FC = () => {
  ...
  const fetchQueue = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await adminService.getApprovedLoads();
      setQueue(data);
    } catch (error) {
      console.error("Failed to load approved queue", error);
    } finally {
      setIsLoading(false);
    }
  }, []);
                ...
                {isLoading ? (
                  ...
                  filteredQueue.map((load, idx) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "16px 24px", verticalAlign: "top" }}>
                        <span style={{ background: "#ECFDF5", color: "#10B981", border: "1px solid #A7F3D0", padding: "4px 10px", borderRadius: "100px", fontSize: "12px", fontWeight: 600 }}>
                          Approved
                        </span>
                      </td>

...
class MessagesPageClass extends React.Component<Props, State> {
  ...
  // Адаптер для сайдбара
  handleSidebarNavigate = (page: string) => {
    if (page === 'messages') this.props.onNavigate('chat');
    else if (page === 'settings') this.props.onNavigate('settings');
    else if (page === 'create-load') this.props.onNavigate('create-load');
    else if (page === 'dashboard') this.props.onNavigate('orders');
    else if (page === 'my-listings') this.props.onNavigate('my-listings');
    else this.props.onNavigate(page);
  };
  ...
  render() {
    ...
    return (
      <div className="dashboard-page active">
        ...
        <Sidebar onNavigate={this.handleSidebarNavigate} activePage="messages" />
          ...
          <header className="dash-header" style={{ borderBottom: '1px solid #E6E8EE', background: 'white', width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
              <div>
                ...
                <h1 className="dash-title" style={{ fontSize: '24px', fontWeight: 600, color: '#0E1116', marginTop: '4px' }}>Messages</h1>
              </div>
            </div>
          </header>

          <div className="messages-container" style={{ display: 'flex', width: '100%', height: 'calc(100vh - 85px)', margin: 0, padding: 0 }}>
            
            <aside className="msg-list-sidebar" style={{ height: '100%' }}>
            ...
            {activeChat ? (
              <div className="msg-chat-area" style={{ height: '100%' }}>
              ...
              <div className="dash-empty-state">Select a chat to view messages</div>
            )}

            {activeChat && activeDeal && (
              <aside className="msg-deal-sidebar" style={{ height: '100%' }}>

...
// Импортируем только сам сервис
import { loadsService } from '../../services/loadsService';
import { RoutingMap } from '../../components/UI/RoutingMap';

// ИСПРАВЛЕНИЕ: Прописываем интерфейсы локально, чтобы TypeScript сборщика Vite не ругался на отсутствие экспортов
...
export const SearchPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFilteredLoads = async () => {
      setIsLoading(true);
      try {
        // Защита: Если бэкендер еще не добавил searchLoads, используем getAllLoads как запасной вариант
        let data;
        ...
        setLoads([]); // При ошибке выводим пустую таблицу
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="dashboard-page active">
      <Sidebar onNavigate={handleSidebarNavigate} activePage="dashboard" />
        ...
        <header className="dash-header" style={{ padding: '16px 32px', borderBottom: '1px solid #E6E8EE', background: 'white' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
            <div>
              ...
              <h1 className="dash-title" style={{ fontSize: '24px', fontWeight: 600, color: '#0E1116', marginTop: '4px' }}>Load board</h1>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <button 
                ...
                style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: '#3D5AFE', color: 'white', fontWeight: 500, cursor: 'pointer' }}
              >
                + Post load
              </button>
            </div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'white', padding: '24px 32px', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap' }}>
              ...
              <div className="filter-select-wrapper">
                ...
                <input type="date" className="filter-select" value={filters.date || ''} onChange={(e) => setFilters({...filters, date: e.target.value})} style={{ padding: 0, color: filters.date ? '#0E1116' : '#A0AAB9' }} />
              </div>
              <div className="filter-select-wrapper">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <div style={{ fontSize: '13px', color: '#5C6470', display: 'flex', alignItems: 'center', gap: '8px' }}>
                Sort
                <select style={{ border: 'none', background: 'transparent', fontWeight: 600, color: '#0E1116', outline: 'none', cursor: 'pointer' }}>
